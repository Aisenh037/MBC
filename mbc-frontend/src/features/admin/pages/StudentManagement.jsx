...
export default function StudentManagement() {
  ...
  const handleOpenDialog = (student = null, view = false) => {
  ...
  const handleDelete = (id) => {
    ...
    if (window.confirm('Are you sure you want to delete this student?')) {
      deleteMutation.mutate(id, {
        ...
        onError: (err) => notify(err?.response?.data?.error || err?.message || 'Failed to delete student', 'error'),
      });
    }
  };
  ...
  const handleSendReset = (userId) => {
    ...
    sendResetLinkMutation.mutate(userId, {
      ...
      onError: (err) => notify(err?.response?.data?.error || err?.message || 'Failed to send reset link', 'error'),
    });
  };
  ...
  const handleFileUpload = (e) => {
    ...
    bulkImportMutation.mutate(file, {
      ...
      onError: (err) => notify(err?.response?.data?.error || err?.message || 'Import failed', 'error'),
    });
    e.target.value = null;
  };
  ...
  const handleExport = () => {
    exportMutation.mutate(undefined, {
      ...
      onError: (err) => notify(err?.response?.data?.error || err?.message || 'Export failed', 'error'),
    });
  };
  ...
  const columns = [
    { 
      field: 'scholarNo', 
      headerName: 'Scholar No.', 
      flex: 1,
      valueGetter: (params) => params.row.scholarNo || 'N/A'
    },
    { 
      field: 'name', 
      headerName: 'Name', 
    ...
    { 
      ...
      headerName: 'Branch', 
      flex: 1.5, 
      valueGetter: (params) => params.row.branch?.name || params.row.branch || 'N/A' 
    },
    {
      field: 'actions',
      headerName: 'Actions',
      ...
      renderCell: (params) => (
        <Stack direction="row" spacing={0.5}>
          ...
          <Tooltip title="Send Reset Link">
            <IconButton 
              onClick={() => handleSendReset(params.row.user?._id)} 
              size="small"
              color="secondary"
            >
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 3 }}>
        ...
        <Stack direction="row" spacing={2}>
          ...
          <Button
            ...
            sx={{ minWidth: 150 }}
          >
            {bulkImportMutation.isLoading ? <CircularProgress size={24} /> : 'Import CSV'}
          ...
          <Button 
            ...
            sx={{ minWidth: 150 }}
          >
            {exportMutation.isLoading ? <CircularProgress size={24} /> : 'Export CSV'}
          </Button>
        </Stack>
      </Stack>
